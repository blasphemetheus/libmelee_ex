...
| Universal batch: fox trot, moonwalk, wavedash OOS, shine turnaround, drillshine, JC grab, crouch cancel, powershield, shield drop (BF), falco double laser | `mix test --only dolphin_universal` | ExiAI headless (flush-patched) | ~12s, no window; standing catch out of a dash, GuardReflect on a tracked laser, Pass through a platform, CC peak 0.0 vs 9.59 |
| Character kits: Peach float, Falcon gentleman, Marth pivot fsmash, Samus SH missile, ICs grab desync | `mix test --only dolphin_characters` | ExiAI headless (flush-patched) | ~20s, 5 boots; Nana blizzards solo while Popo holds the grab, jab3 without rapid |
