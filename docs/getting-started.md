...
`team:`, and explicit `p2:`/`p3:`/`p4:` port specs — or run SEVERAL
bots in one match with `Melee.Bot.run_many/2` (teammates, or a full
bot free-for-all), with `Melee.GameState.allies/2` and `enemies/2`
splitting the field for each policy. `Melee.Match.play/2`
...
| Custom Rules: stocks, timer, Team Attack (behaviorally), Pause, items | `mix test --only dolphin_rules` | ExiAI headless | ~40s, no window; stock=1/timer=300s from GAME_START, ally damage 9% vs 0.0% across the toggle, pause-off refuses the LRAS quit, a Poke-Ball-only game spawns exactly containers + poke balls, and `boot_rules:` starts a match pre-configured with zero menu taps |
...
| `Melee.Tech`, all tiers: hops, wavedash, dash dance, SHFFL+L-cancel, multishine, pivot, waveland, waveshine, SH laser, ledgedash (with galint), Ness DJC | `mix test --only dolphin_movement` | ExiAI headless (flush-patched) | ~35s, no window; 8/8 wavedashes at 21.1 travel, lag 6 vs 14, 4/4 waveshines, ledgedash lands invincible |
| Tier 4 hit response: SDI/DI A/B vs control under a scripted falco launcher, live ground tech at 58% | `mix test --only dolphin_defense` | ExiAI headless (flush-patched) | ~4s, no window; SDI slides 22.7 units in hitlag vs 0.0, DI bends knockback dx to −9.2 vs 0.5, tech-in-place (0xC7) on the fall |
| Mewtwo kit: DJC nair, shadow ball store + fire, teledgehog | `mix test --only dolphin_mewtwo` | ExiAI headless (flush-patched) | ~6s, no window; DJC apex 2.72, charge stored via 0x158, ball released (0x159, projectile seen), CliffCatch on FD's lip |
| Universal batch: fox trot, moonwalk, wavedash OOS, shine turnaround, drillshine, JC grab, crouch cancel, powershield, shield drop (BF), falco double laser | `mix test --only dolphin_universal` | ExiAI headless (flush-patched) | ~12s, no window; standing catch out of a dash, GuardReflect on a tracked laser, shine-grab chain, Pass through a platform, Yoshi egg shield mapped, CC peak 0.0 vs 9.59 |
| Character kits: Peach float, Falcon gentleman + instant RAR, Marth pivot fsmash, Samus SH missile, ICs grab desync | `mix test --only dolphin_characters` | ExiAI headless (flush-patched) | ~25s, 5 boots; Nana blizzards solo while Popo holds the grab, jab3 without rapid, RAR bair at 38.7 drift |
| Research: Peach FC + Samus missile cancel PROVEN (actionability probe), super wavedash 126 units, Ness yo-yo glitch at 28.8 range, Mewtwo teleport edge-cancel, tipper, thunders | `mix test --only dolphin_research` | ExiAI headless (flush-patched) | ~45s, 6 boots; the landing-animation-vs-lag artifact corrected two earlier verdicts |
